...
def get_num_contains(G, cur_node):
    return 1 + sum(G[cur_node][s]["count"]*get_num_contains(G, s) for s in G.successors(cur_node))


# ============= Input Data ============= #
raw_data = Path("./inputs/07/input.txt").read_text()
# Let's learn a new library and try to solve the puzzle by using networkx. In the end it might be easier to do it by hand...
edges = []
for line in raw_data.splitlines():
    ...
    if not cs.startswith("no other"):
        for c in cs.split(", "):
            ...
            edges.append((p, grps[1], {'count': int(grps[0])}))
G = nx.DiGraph(edges)
